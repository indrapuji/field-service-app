...
const DetailScreen = ({ route, navigation }) => {
  ...
  const handdleFraud = (fraud) => {
    ...
    setValue({ ...value, edukasi_fraud: fraud.key });
  };
  const handdleGestun = (gestun) => {
    setGestunChecked(gestun.key);
    setValue({ ...value, edukasi_gestun: gestun.key });
  };

  const handdlesurcharge = (surcharge) => {
    setSurchargeChecked(surcharge.key);
    setValue({ ...value, edukasi_surcharge: surcharge.key });
  };
  const handdlekartu_asing = (kartu_asing) => {
    setAsingChecked(kartu_asing.key);
    setValue({ ...value, edukasi_kartu_asing: kartu_asing.key });
  };
  const handdledouble_transaksi = (double) => {
    setDoubleChecked(double.key);
    setValue({ ...value, edukasi_double_transaksi: double.key });
  };
          ...
          <View style={{ flex: 1, backgroundColor: '#e3fdfd' }}>
            <View style={{ flex: 1 }}>
              ...
              >
                ...
                <Text style={{ marginHorizontal: 30, textAlign: 'center' }}>{itemData.alamat}</Text>
              </View>
              <TouchableOpacity
                style={{
                  position: 'absolute',
              ...
              <ScrollView>
                <View style={{ marginBottom: 20, marginHorizontal: 20 }}>
                  <View>
                    <HeaderCategory title={'Data Merchant'} />
                    <InputComponent
                      lable={'Nama PIC'}
                    ...
                    />
                    <InputComponent lable={'TID'} value={itemData.tid} editable={false} />
                    <InputComponent lable={'MID'} value={itemData.mid} editable={false} />
                    <InputComponent lable={'Serial Number'} value={value.sn_edc} onChangeText={(text) => setValue({ ...value, sn_edc: text })} />
                    <InputComponent lable={'ICCID SIM'} value={value.sim_card} onChangeText={(text) => setValue({ ...value, sim_card: text })} />
                  ...
                  {/* Detail EDC */}
                  <View>
                    <HeaderCategory title={'Detail EDC'} />
                    <View style={{ marginTop: 20 }}>
                  ...
                  <View>
                    <HeaderCategory title={'Foto Merchant'} />
                    <View style={{ marginTop: 20 }}>
