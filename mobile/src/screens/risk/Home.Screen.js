...
import { StatusBar, View, Text, SafeAreaView, TextInput, TouchableOpacity, ActivityIndicator, RefreshControl, FlatList } from 'react-native';
...
const HomeScreen = () => {
  ...
  const getJobOrder = async () => {
    try {
      ...
      const { data } = await axios({
        method: 'get',
        url: `${host}/job-orders/all?tipe=Risk&status=Assign`,
        headers: { token },
      });
      setList(data.data);
  ...
  const getMoreOrder = async () => {
    try {
      ...
      const { data } = await axios({
        method: 'get',
        url: `${host}/job-orders/all?tipe=Risk&status=Assign&page=${currentPage + 1}`,
        headers: { token },
      });
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 10 }}>
            <FlatList
              ...
              renderItem={({ item, index }) => <CardList source={'home'} update={update} location={location} item={item} />}
